...
//				File rcPack200 = new File(getTempDirectory(), "tmp_" + System.currentTimeMillis() + "." + coreExtPack);
				...
				try {
					errorList.add("getResourceAsStream /core/core." + coreExt);
					is = new TP().getClass().getResourceAsStream("/core/core." + coreExt);
					if (is == null) {
						errorList.add("getResourceAsStream /core/core." + coreExtPack);
...
//	    e.printStackTrace();
			throw new RuntimeException("Errors in CFMLEngineFactory: initEngine: " + errorList.toString() + "\nException\n" + e.getMessage() + "\n" + sb.toString());
			//	    throw new ServletException(e);
//		return;
		}
	...
	public Felix getFelix(final File cacheRootDir, Map<String, Object> config) throws BundleException {
		...
		// Log Level
		int logLevel = 4; // 1 = error, 2 = warning, 3 = information, and 4 = debug
...
//		if (Util.isEmpty(storageClean))
			config.put(Constants.FRAMEWORK_STORAGE_CLEAN, Constants.FRAMEWORK_STORAGE_CLEAN_ONFIRSTINIT); //"none");//
	...
	private File deployBundledBundle(File bundleDirectory, String symbolicName, String symbolicVersion) {
		...
		if (is != null) {
			log(Logger.LOG_DEBUG, "found /bundles/" + osgiFileName + " in lucee.jar");
		} else {
			log(Logger.LOG_INFO, "could not find /bundles/" + osgiFileName + " in lucee.jar");
		}
		...
		if (is == null) {
			...
			if (is != null) {
				log(Logger.LOG_DEBUG, "found /bundles/" + osgiFileName + pack20Ext + " in lucee.jar");
			} else {
				log(Logger.LOG_INFO, "could not find /bundles/" + osgiFileName + pack20Ext + " in lucee.jar");
			}
		}
		if (is != null) {
			File temp = null;
			try {
				...
				// this method might be needed under first time conditions
				FileInputStream fileIS=new FileInputStream(temp);
				Util.copy(new BufferedInputStream(fileIS), new FileOutputStream(trg), true, true);

//				boolean renameSuccess = temp.renameTo(trg);
//				if (!renameSuccess) {
//					if (trg == null) {
//						throw new RuntimeException("Java SecurityManager - trg path was null: " + trg.getPath());
//					}
//					if ((temp.getPath().indexOf('\u0000') >= 0)) {
//						throw new RuntimeException("Java SecurityManager - invalid temp path: |" + temp.getPath() + "|" + (temp.getPath().indexOf('\u0000')));
//					}
//					if ((trg.getPath().indexOf('\u0000') >= 0)) {
//						throw new RuntimeException("Java SecurityManager - invalid destination path: |" + trg.getPath() + "|" + (trg.getPath().indexOf('\u0000')));
//					}
//					throw new RuntimeException("Java SecurityManager error. Unable to rename " + temp.getAbsolutePath() + " exists:" + temp.exists() + " to " + trg.getAbsolutePath() + " exists: " + trg.exists());
//				}
//				log(Logger.LOG_DEBUG, "adding bundle [" + symbolicName + "] in version [" + symbolicVersion + "] to [" + trg + "] renameSuccess: " + renameSuccess);
	...
	private boolean removeLatestUpdate() throws IOException, ServletException {
		...
		for (final File patche : patches)
			if (patch == null || Util.isNewerThan(toVersion(patche.getName(), VERSION_ZERO), toVersion(patch.getName(), VERSION_ZERO)))
				patch = patche;
	...
	private File _getResourceRoot() throws IOException {
		...
		if (root == null)
			throw new IOException("can't locate the root of the servlet container, please define a location (physical path) for the server configuration"
