...
import static lucee.transformer.bytecode.op.OpString.METHOD_CONCAT;
...
public class OpUnary extends ExpressionBase implements ExprDouble {
	...
	public Type _writeOut(BytecodeContext bc, int mode) throws TransformerException {
		...
		if ((scope == null && size > 1) || (scope != null && size > 0)) {
			...
			Collection.Key fieldKey=new KeyImpl(((DataMember) last).getName().toString());

			String localFieldName=bc.getLocalField(fieldKey, false);
			if(var.getScope() == ScopeSupport.SCOPE_JETENDO && localFieldName!=null){
				if (type == Factory.OP_UNARY_PRE) {
					if (operation == Factory.OP_UNARY_CONCAT) {
						bc.loadFieldClass();
						bc.loadFieldClass();
						...
						adapter.invokeVirtual(Types.STRING, METHOD_CONCAT);
						adapter.putField(bc.fieldClazzType, localFieldName, Types.OBJECT);
						return Types.VOID;
					} else {
						int localIndex = adapter.newLocal(Types.DOUBLE);
						bc.loadFieldClass();
						bc.loadFieldClass();
						adapter.getField(bc.fieldClazzType, localFieldName, Types.OBJECT);
						...
						if (operation == Factory.OP_UNARY_PLUS) {
							adapter.visitInsn(Opcodes.DADD);
						} else if (operation == Factory.OP_UNARY_MINUS) {
							adapter.visitInsn(Opcodes.DSUB);
						} else if (operation == Factory.OP_UNARY_DIVIDE) {
							adapter.visitInsn(Opcodes.DDIV);
						} else if (operation == Factory.OP_UNARY_MULTIPLY) {
							adapter.visitInsn(Opcodes.DMUL);
						}else{
							throw new TransformerException("This type of operation, " + operation +", is not implemented for local fields.", value.getStart());
						}
						...
						adapter.putField(bc.fieldClazzType, localFieldName, Types.OBJECT);
						if (mode == MODE_REF) {
							adapter.loadLocal(localIndex, Types.DOUBLE); // restore the original value for returning it.
...
//					}
					int localIndex = adapter.newLocal(Types.DOUBLE_VALUE);
					bc.loadFieldClass();
					bc.loadFieldClass();
					adapter.getField(bc.fieldClazzType, localFieldName, Types.OBJECT);
					adapter.invokeStatic(Types.CASTER, Methods.METHOD_TO_DOUBLE_VALUE);
					if (mode == MODE_REF) {
						adapter.storeLocal(localIndex);
						adapter.loadLocal(localIndex, Types.DOUBLE_VALUE);
					}

					adapter.visitInsn(Opcodes.DCONST_1);
					if (operation == Factory.OP_UNARY_PLUS) {
						adapter.visitInsn(Opcodes.DADD);
					} else if (operation == Factory.OP_UNARY_MINUS) {
						adapter.visitInsn(Opcodes.DSUB);
					...
					adapter.putField(bc.fieldClazzType, localFieldName, Types.OBJECT);
					if (mode == MODE_REF) {
						adapter.loadLocal(localIndex); // restore the original value for returning it.
						adapter.invokeStatic(Types.CASTER, Methods.METHOD_TO_DOUBLE_FROM_DOUBLE);
						return Types.DOUBLE;
					}
					return Types.VOID;
				...
				if (type == Factory.OP_UNARY_POST) {
					...
					} else if (operation == Factory.OP_UNARY_CONCAT) {
