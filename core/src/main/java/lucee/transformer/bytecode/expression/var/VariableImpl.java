...
public class VariableImpl extends ExpressionBase implements Variable {
	...
	private static java.lang.reflect.Method getMethod(Class<?> clazz, String memberName) {
	...
	private static Type writeOutGetScopeMethod(BytecodeContext bc, GeneratorAdapter adapter, Class<?> clazz, int scope, UDF udf) throws TransformerException {
		...
		java.lang.reflect.Method reflectMethod = getMethod(clazz, memberName);
		if (reflectMethod == null) {
			throw new RuntimeException("There is no method named: " + memberName + " in " + clazz.getCanonicalName());
		}
		// verify method is accessible
		int modifiers = reflectMethod.getModifiers();
//	    if(!method.isAccessible()){
		if (Modifier.isPrivate(modifiers) || Modifier.isProtected(modifiers)) {
			throw new IllegalAccessError(memberName + " is not an accessible method in " + clazz.getCanonicalName());
		}
...
//			adapter.visitMethodInsn(Opcodes.INVOKESTATIC, "lucee.runtime.type.scope.JetendoImpl", "memberBoolFuncStatic", "()Ljava/lang/Boolean;", false);
...
//			adapter.visitMethodInsn(Opcodes.INVOKEVIRTUAL, clazzType.getClassName(), methodName, "()Ljava/lang/Boolean;", false);
		...
		ASMProxyFactory.boxPrimitive(adapter, methodClazz);
		return Types.OBJECT;//methodType;
	}
...
//		TypeScope.invokeScope(adapter, scope);
	...
	private static String getName(Expression expr) throws TransformerException {
		...
		if (name == null)
			throw new TransformerException("cannot extract a string from a object of type [" + expr.getClass().getName() + "]", null);
		return name;
	}
