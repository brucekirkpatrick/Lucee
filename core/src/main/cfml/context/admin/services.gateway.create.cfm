...
<cfoutput>
	...
	<form onerror="customError" action="#request.self#?action=#url.action#&action2=create#iif(isDefined('url.id'),de('&id=##url.id##'),de(''))#" method="post">
		<input type="hidden" name="name" value="#listLast(getMetaData(driver).name,'.')#">
		<input type="hidden" name="class" value="#entry.class#">
		<input type="hidden" name="cfcPath" value="#entry.cfcPath#">
			...
			<tbody>
				...
				<cfif driver.getListenerCfcMode() NEQ "none">
					<tr>
						...
						<td><input type="text" name="listenerCfcPath" value="#entry.listenerCfcPath#" required="#driver.getListenerCfcMode() EQ "required"#" class="large" message="Missing value for field listener CFC Path">
						<div class="comment">
